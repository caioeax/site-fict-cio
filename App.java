...
public class App {
    ...
    public static void main(String[] args) throws IOException {
        ...
        byte[] hashedPassword;
        // Evite usar String para senhas, use char[] ou byte[]. A String é imutável e pode ser mantida na memória por mais tempo do que o necessário, tornando-a vulnerável
        // a ataques de memória. Use char[] ou byte[] para senhas, pois eles podem ser limpos da memória após o uso. Isso reduz a janela de oportunidade para um invasor acessar a senha.
        // java.util.Arrays.fill(password, '0'); // Limpa a senha após o uso
        char[] password = { '1', '2', '3', '4', '5', '6' }; // Senha a ser testada
        // Testando algoritmos de hash com message digest MD5, SHA-1, SHA-256, SHA-512 e comparando o tempo de execução
        ...
        String algorithm = "PBKDF2WithHmacSHA512"; // Testando algoritmos de hash com PBKDF2 e comparando o tempo de execução
        try {
            start = Duration.ofNanos(System.nanoTime());
            hashedPassword = PasswordHashing.hashPasswordWithPBKDF2(password, salt, algorithm, iterations, keyLength);
            end = Duration.ofNanos(System.nanoTime());
            System.out.println(String.format(msg, algorithm, end.minus(start).toNanos(), encoder.encodeToString(hashedPassword)));
        ...
        System.out.println(
                String.format(msg, algorithm, end.minus(start).toNanos(), encoder.encodeToString(hashedPassword)));
